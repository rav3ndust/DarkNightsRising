...
main () {
	...
	echo "${line1}" & say "${line1}"; sleep 1
	echo "${line2}" & say "${line2}"; sleep 1
	echo "${line3}" & say "${line3}"; read -p "Please enter your name: " NAME
	...
	# - - - - - - - - - - - - - - - - - - 2
	echo "${line4}" & say "${line4}"; sleep 1
	echo "${line5}" & say "${line5}"; read -p "Please enter your character's name: " CHARNAME
	line6="Excellent. What race will $CHARNAME hail from? Be creative. Please enter it to continue."
	echo "${line6}" & say "${line6}"; read -p "Please enter desired character race: " RACE
	line7="Cool, so $CHARNAME is from the $RACE race."
	echo "${line7}" & say "${line7}"; sleep 1
	line8="Next, let us decide where $CHARNAME hails from. What should their home kingdom be called? Please enter it to continue."	
	echo "${line8}" & say "${line8}"; read -p "Please enter desired character's home: " PLACE
	...
	echo "${line9}" & say "${line9}"; sleep 1
	line10="Are you happy with this character, $NAME? Type 'yes' or 'no' to continue."
	echo "$line10" & say "${line10}"; read -p "Are you happy with this configuration? " PLAYERCONFIG_FINALISE
